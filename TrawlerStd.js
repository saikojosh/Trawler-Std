...
module.exports = class TrawlerStd {
  ...
  init (finish) {
    ...
    this.log.debug(`   Source Change Ignored: ${sourceChangeIgnored.length} item(s)`);
    for (let i = 0, ilen = this.config.trawler.sourceChange.ignored.length; i < ilen; i++) {
      this.log.debug(`      • ${this.config.trawler.sourceChange.ignored[i]}`);
    }
    this.log.debug(`   Source Change Watched: ${sourceChangeWatched.length} item(s)`);
    for (let w = 0, wlen = this.config.trawler.sourceChange.watched.length; w < wlen; w++) {
      this.log.debug(`      • ${this.config.trawler.sourceChange.watched[w]}`);
    }
